...
class TaggableManager(RelatedField, Field):
    ...
    def deconstruct(self):
        ...
        # rel.to renamed to rel.model in Django 1.9
        if VERSION >= (1, 9):
            if isinstance(self.rel.model, six.string_types):
                kwargs['to'] = self.rel.model
            else:
                kwargs['to'] = '%s.%s' % (self.rel.model._meta.app_label, self.rel.model._meta.object_name)
        else:
            ...
            if VERSION >= (1, 9):
                if isinstance(self.rel.model, six.string_types):
                    def resolve_related_class(field, model, cls):
                        field.rel.model = model
                    add_lazy_relation(cls, self, self.rel.model, resolve_related_class)
            else:
                if isinstance(self.rel.to, six.string_types):
                    def resolve_related_class(field, model, cls):
                        field.rel.to = model
    ...
    def post_through_setup(self, cls):
        ...
        if VERSION >= (1, 9):
            if not self.rel.model:
                self.rel.model = self.through._meta.get_field("tag").rel.model
        else:
            if not self.rel.to:
    ...
    def m2m_reverse_target_field_name(self):
        # rel.to renamed to rel.model in Django 1.9
        if VERSION >= (1, 9):
            return self.rel.model._meta.pk.name
        else:
